...
VIEW `drugwholesale`.`drugs_view` AS
    ...
    FROM
        (((`drugwholesale`.`drugs`
        JOIN `drugwholesale`.`companies` ON ((`drugwholesale`.`drugs`.`Comp_ID` = `drugwholesale`.`companies`.`Comp_ID`)))
        JOIN `drugwholesale`.`drug_type` ON ((`drugwholesale`.`drugs`.`Type_ID` = `drugwholesale`.`drug_type`.`Type_ID`)))
        JOIN `drugwholesale`.`demography` ON ((`drugwholesale`.`drugs`.`Demo_ID` = `drugwholesale`.`demography`.`Demo_ID`)))